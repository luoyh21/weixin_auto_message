...
生成静态合辑页、中文总结详情页与卡片配图，并用 LLM 将精选新闻的已有概览二次压缩成
无省略号的完整短句。群发文本包含 4 条国际新闻、1 条 TechPort、最多 2 次未来发射及
1 条公众号标题，不再追加“查看本周全部动态”；附件仅保留一张可正常转发的小程序总览卡片。
任务名称采用“2026-7月第3周｜本周航天概览”格式，通过
`externalcontact/add_msg_template` 给 `WECOM_EXTERNAL_SENDER` 创建待发送任务。接口不会直接触达客户，
